...
export default function DeckDetailPage() {
  ...
  async function createCard(e) {
    ...
    try {
      await CardsApi.create(deckId, {
        front: front.trim(),
        back: back.trim(),
        context_sentence: sentence || null,
      });
      setFront("");
      setBack("");
      setContextSentence("");
  ...
  function startEdit(card) {
    ...
    setEditContextSentence(card.context_sentence ?? card.example_sentence ?? "");
  }
  ...
  function cancelEdit() {
    ...
    setEditFront("");
    setEditBack("");
    setEditContextSentence("");
  }

  async function saveEdit(card) {
    if (!editFront.trim() || !editBack.trim()) return;
    const sentence = editContextSentence.trim();
    setSavingId(card.id);
    setCardsError("");
    try {
    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        <form onSubmit={createCard} className="mt-4 grid gap-3">
          ...
          <Input placeholder="Translation" value={back} onChange={(e) => setBack(e.target.value)} />
          <Input placeholder="Context sentence (optional)" value={contextSentence} onChange={(e) => setContextSentence(e.target.value)} />
            ...
            <Card key={card.id}>
              {isEditing ? (
                <div className="grid gap-3">
                  ...
                  <Input value={editBack} onChange={(e) => setEditBack(e.target.value)} placeholder="Translation" />
                  <Input value={editContextSentence} onChange={(e) => setEditContextSentence(e.target.value)} placeholder="Context sentence (optional)" />
                  <div className="flex flex-wrap gap-2">
                ...
                <div className="space-y-2">
                  <div>
                    ...
                    {card.context_sentence ? <p className="mt-1 text-sm text-gray-600">{card.context_sentence}</p> : null}
                  </div>
                  <div><p className="text-xs text-gray-500">Translation</p><p className="text-gray-800">{card.back}</p></div>
